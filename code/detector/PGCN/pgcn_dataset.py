...
class PGCNVideoRecord:
    def __init__(self, prop_record, i):
...
def _prepare_iou_dict2(video_list, fg_iou_thresh, gt_as_fg, incomplete_iou_thresh, bg_iou_thresh,
...
class PGCNDataSet(data.Dataset):
    ...
    def __init__(self, dataset_configs, graph_configs, prop_file, prop_dict_path, ft_path, exclude_empty=True,
        ...
        if os.path.exists(self.prop_dict_path):
            ...
            dicts = joblib.load(self.prop_dict_path)
            ...
            if self.test_mode:
                ...
                # self.prop_dict, self.act_iou_dict, self.act_dis_dict = \
                #    _prepare_iou_dict2(self.video_list, self.fg_iou_thresh, self.gt_as_fg, self.incomplete_iou_thresh,
                #                      self.bg_iou_thresh, self.bg_coverage_thresh, self.incomplete_overlap_thresh)
            joblib.dump([self.act_iou_dict, self.act_dis_dict, self.prop_dict], self.prop_dict_path)
    ...
    def _parse_prop_file(self, stats=None):
        ...
        self.video_list = [PGCNVideoRecord(p, i) for i, p in enumerate(prop_info)]
