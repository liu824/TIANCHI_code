...
def get_configs(dataset):
    data = yaml.load(open('detector/PGCN/data/dataset_cfg.yaml', 'r'), Loader=yaml.RoundTripLoader)
    return data[dataset]
...
def get_and_save_args(parser):
    ...
    default_config = yaml.load(open('detector/PGCN/data/dataset_cfg.yaml', 'r'), Loader=yaml.RoundTripLoader)[dataset]
    ...
    for k, v in current_config.items():
        if k in default_config:
            if (v != default_config[k]) and (v is not None):
                ...
                default_config[k] = v
    yaml.dump(default_config, open('detector/PGCN/data/current_configs.yaml', 'w'), indent=4, Dumper=yaml.RoundTripDumper)
